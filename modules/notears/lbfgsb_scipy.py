...
class LBFGSBScipy(torch.optim.Optimizer):
    ...
    def step(self, closure):
        ...
        def wrapped_closure(flat_params):
            ...
            flat_params = torch.from_numpy(flat_params).to(dtype=torch.double)
            self._distribute_flat_params(flat_params)
            loss = closure()
            loss = loss.item()
            flat_grad = self._gather_flat_grad().cpu().detach().numpy().astype(np.float64)
            return loss, flat_grad

        # --- Initial parameters as float64
        initial_params = self._gather_flat_params()
        initial_params = initial_params.cpu().detach().numpy().astype(np.float64)

        # --- Bounds as float64-safe
        ...
        # --- Set optimized parameters (double)
        final_params = torch.from_numpy(sol.x).to(dtype=torch.double)
